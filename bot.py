...
# Randomisation library from python
import random 
#from choomclass import Flatline
...
@bot.command()
async def choomadd(ctx, a:int, b:int):
    await ctx.send(f'Yo! {a} + {b} = {a + b}, math stuff!')

#   Broken for now
#@bot.command()
#async def bang(ctx,*,reason:Flatline()):
#    await ctx.send(reason)
    
@bot.command()
async def bang(ctx, user:discord.Member, reason="No reason"):
    endcomment = ["rip", "brutal", "sick"]
    randcomment = random.choice(endcomment)
    await ctx.channel.send(f'{ctx.author} flatlined {user.mention}, because: {reason}, {randcomment}')
